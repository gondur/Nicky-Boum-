...
uint8 inp_direction_mask, inp_fire_button, inp_fire_button_num_cycles_pressed;
...
static void inp_record(int inp_direction_mask, int inp_fire_button) {
	if (file_slot >= 0) {
		uint8 b = inp_direction_mask;
		if (inp_fire_button) {
			b |= 0x80;
		}
...
void inp_start_level(int level_num) {
	...
	case IRS_RECORD:
		...
		if (file_slot >= 0) {
			uint8 tmp[2];
			write_uint16BE(tmp, random_seed);
